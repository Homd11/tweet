...
def render():
    ...
    if st.button("🔍 Compare Models", type="primary"):
        ...
        with col1:
            ...
            st.markdown(
                f"<div style='text-align:center; font-size:1.5em; color:{sentiment_colors.get(lr_label, '#333')}; font-weight:bold;'>"
            ...
            for idx, label in enumerate(le.classes_):
                prob = float(lr_proba[idx])
                st.markdown(f"**{label}**: {prob:.2%}")

        with col2:
            st.markdown("### 🧠 BERT")

            use_bert = st.checkbox("Enable BERT (requires transformers + model download)", value=False, key="bert_toggle")
            ...
            if use_bert:
                try:
                    ...
                    if language == "arabic" or is_arabic_text(text_input):
                        bert_model = BERTClassifier(model_name="aubmindlab/bert-base-arabertv02", is_arabic=True)
                    else:
                    ...
                    with st.spinner("Loading BERT model... This may take a while on first run."):
                        bert_model.load_pretrained()

                    bert_result = bert_model.predict_with_aspects(text_input)
                    ...
                    st.markdown(
                        f"<div style='text-align:center; font-size:1.5em; color:{sentiment_colors.get(bert_label, '#333')}; font-weight:bold;'>"
                    ...
                    st.info("Make sure 'transformers' and 'torch' are installed, and you have internet access for model download.")
            else:
                st.info("BERT is disabled. Check the checkbox above to enable it.")
        ...
        if use_bert:
            ...
            comparison = pd.DataFrame({
                ...
                "Sentiment": [lr_label, bert_label if use_bert else "N/A"],
                "Confidence": [f"{lr_confidence:.2%}", f"{bert_confidence:.2%}" if use_bert else "N/A"],
            })
